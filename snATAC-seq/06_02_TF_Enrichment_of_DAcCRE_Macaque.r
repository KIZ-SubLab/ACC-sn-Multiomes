...
for temp_celltype in celltype_list:
    temp_diff = pd.read_csv('../DAcCRE/'+temp_celltype+'_Macaque.txt',
                            sep='\t', header=None)
...
DiffPeak.to_csv('DiffPeak_Macaque.txt', sep='\t')
#######################################################


# Add DAcCRE Information
DiffPeak_df = read.delim('DiffPeak_Macaque.txt', sep='\t')#, row.names=1)
